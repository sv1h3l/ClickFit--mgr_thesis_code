...
const TwoColumnsPage = ({ firstColumnWidth, secondColumnWidth, firstColumnHeight, secondColumnHeight, firstColumnChildren, secondColumnChildren }: TwoColumnsPageProps) => {
	return (
		<Box className="flex justify-center h-content">
			<Box
				className={`${firstColumnWidth ? firstColumnWidth : "w-1/2"} ${firstColumnHeight ? firstColumnHeight : "h-full"} ${secondColumnHeight && "rounded-br-3xl"} 
						bg-white relative  border-l-2 border-gray-200 `}>
				{firstColumnChildren}
			</Box>
			<Box
				className={`${secondColumnWidth ? secondColumnWidth : "w-1/2"} ${secondColumnHeight ? secondColumnHeight : "h-full"}
