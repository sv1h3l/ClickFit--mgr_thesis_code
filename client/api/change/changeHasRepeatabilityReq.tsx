...
export const changeHasRepeatabilityReq = async (props: Props): Promise<GenericResponse<null>> => {
	const serverIp = process.env.NEXT_PUBLIC_SERVER_IP || "localhost:5000";

	try {
		const response = await fetch(`http://${serverIp}/api/change-has-repeatability`, {
			method: "POST",
			credentials: "include",
			headers: {
