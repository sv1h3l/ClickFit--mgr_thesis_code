...
export const changeSportHasAutomaticPlanCreationReq = async (props: Props): Promise<GenericResponse<null>> => {
	const serverIp = process.env.NEXT_PUBLIC_SERVER_IP || "localhost:5000";

	try {
		const response = await fetch(`http://${serverIp}/api/change-sport-has-automatic-plan-creation`, {
			method: "POST",
			credentials: "include",
			headers: {
