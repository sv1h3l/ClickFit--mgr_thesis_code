...
export const createSportDetailLabReq = async (props: Props): Promise<GenericResponse<{ sportDetailLabId: number; sportDetailValId: number }>> => {
	const serverIp = process.env.NEXT_PUBLIC_SERVER_IP || "localhost:5000";

	try {
		const response = await fetch(`http://${serverIp}/api/create-sport-detail-label`, {
			method: "POST",
			credentials: "include",
			headers: {
