...
export const getVisitedUserCategoriesWithExercisesReq = async ({ props }: { props: Props }): Promise<GenericResponse<Category[]>> => {
	const serverIp = process.env.NEXT_PUBLIC_SERVER_IP || "localhost:5000";

	try {
		const response = await fetch(`http://${serverIp}/api/get-visited-user-categories-and-exercises?sportId=${props.sportId}&visitedUserId=${props.visitedUserId}`, {
			method: "GET",
			credentials: "include",
			headers: {
