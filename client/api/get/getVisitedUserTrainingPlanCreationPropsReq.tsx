...
export const getVisitedUserTrainingPlanCreationPropsReq = async (props: Props): Promise<GenericResponse<Res>> => {
	const serverIp = process.env.NEXT_PUBLIC_SERVER_IP || "localhost:5000";

	try {
		const response = await fetch(`http://${serverIp}/api/get-visited-user-training-plan-creation-props?sportId=${props.sportId}&visitedUserId=${props.visitedUserId}`, {
			method: "GET",
			headers: {
