...
import { changeGraphMod } from "../../models/change/changeGraphMod";
...
export const changeGraphCont = async (req: Request, res: Response): Promise<void> => {
	...
	try {
		...
		if (dbResult.status === GenEnum.SUCCESS) {
			if (changedHasDate) {
				...
				const resChange = await changeXAxisDateGraphValuesMod({ graphId, formattedDate, isDefGraph });

				if (resChange.status === GenEnum.FAILURE) {
					res.status(resChange.status).json({ message: resChange.message });
					return;
				}
			}
			if (!hasGoals) {
				const resDelete = await deleteIsGoalOfGraphValuesMod({ graphId, isDefGraph });

				if (resDelete.status === GenEnum.FAILURE) {
