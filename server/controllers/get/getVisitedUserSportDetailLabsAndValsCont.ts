...
export const getVisitedUserSportDetailLabsAndValsCont = async (req: Request, res: Response): Promise<void> => {
	...
	try {
		...
		let resVals = await getSportDetailValsMod({ sportId: sportIdNumber, userId : visitedUserId });
		...
		if (missingLabels) {
			...
			const resCreateVals = await createMissingSportDetailValsMod({ sportId: sportIdNumber, missingSportDetailIds, userId : visitedUserId });
			...
			resVals = await getSportDetailValsMod({ sportId: sportIdNumber, userId : visitedUserId });
		}
